...
def aa0V(request):
    context = {}
    return render(request, 'aa0.html', context)

def aa1V(request):
    laboratorios = Laboratorio.objects.all()
    return render(request, 'aa1.html', {'laboratorios': laboratorios})
...
def editar_laboratorio_view(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('/aa1')  # Redireccionar a la lista de laboratorios después de guardar
    else:
...
def eliminarlaboratorio_view(request, id):
    laboratorio = get_object_or_404(Laboratorio, id=id)  # Obtener el objeto Auto con el id especificado
    laboratorio.delete()
